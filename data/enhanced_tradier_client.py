...
class EnhancedTradierClient:
    ...
    def __init__(self, api_token: str, sandbox: Optional[bool] = None):
        ...
        if sandbox is None:
            ...
            try:
                ...
                self.sandbox = False
                logger.warning(
                    "Config non disponible, utilisation de la production par défaut"
                )
        else:
        ...
        logger.info(
            f"Enhanced Tradier client initialisé (environnement: {environment})"
        )
    ...
    def get_options_chains(
        ...
        try:
            ...
            # Mise en cache
            self.cache[cache_key] = {"data": contracts, "timestamp": time.time()}
    ...
    def _get_options_quotes_chunk(
        ...
        try:
            ...
            # Mise en cache
            self.cache[cache_key] = {"data": quotes, "timestamp": time.time()}
    ...
    def get_options_expirations(self, symbol: str) -> List[str]:
        ...
        try:
            ...
            if (
                "expirations" in data
                and data["expirations"]
                and "date" in data["expirations"]
            ):
            ...
            # Mise en cache
            self.cache[cache_key] = {"data": expirations, "timestamp": time.time()}
    ...
    def get_multiple_underlying_quotes(
        self, symbols: List[str]
        ...
        try:
                ...
                for quote in quote_data:
                    ...
                    if symbol:
                        quotes_dict[symbol] = {
                            ...
                            "change_percentage": self._safe_float(
            ...
            # Mise en cache
            if quotes_dict:
                self.cache[cache_key] = {"data": quotes_dict, "timestamp": time.time()}

                logger.info(
                    f"✅ Récupéré {len(quotes_dict)} quotes: {', '.join(quotes_dict.keys())}"
                )
            else:
                logger.warning(
                    f"⚠️ Aucune quote récupérée pour: {', '.join(unique_symbols)}"
                )
            ...
            logger.error(
                f"❌ Erreur récupération quotes multiples {unique_symbols}: {e}"
            )
            return {}
    ...
    def _parse_options_chains(
        self, data: Dict[str, Any], underlying: str
    ) -> List[OptionsContract]:
    ...
    def _safe_float(self, value, default=0.0):
        ...
        try:
            ...
            if not self._is_valid_float(result):
                logger.warning(
                    f"⚠️ Valeur float invalide détectée: {result}, utilisation de la valeur par défaut {default}"
                )
                return default
            return result
    ...
    def _extract_greek(self, option_data: Dict[str, Any], greek_name: str) -> float:
        ...
        if "greeks" in option_data and option_data["greeks"] and greek_name == "delta":
            logger.debug(
                f"   🔍 DEBUG: Données greeks brutes pour {symbol}: {option_data['greeks']}"
            )
            ...
            if isinstance(greeks, dict) and greek_name in greeks:
                ...
                if raw_value is not None:
                    try:
                        ...
                        logger.debug(
                            f"   ✅ {greek_name.upper()} {symbol}: {greek_value} (depuis greeks)"
                        )
                        return greek_value
                    except (ValueError, TypeError):
                        logger.warning(
                            f"   ⚠️ Impossible de convertir {greek_name.upper()} pour {symbol}: {raw_value}"
                        )
        ...
        if direct_value is not None:
            try:
                ...
                logger.debug(
                    f"   ✅ {greek_name.upper()} {symbol}: {greek_value} (direct)"
                )
                return greek_value
            except (ValueError, TypeError):
                pass
    ...
    def _is_greek_value_valid(self, greek_name: str, value: float, symbol: str) -> bool:
        ...
        if greek_name == "delta":
            ...
            return (
                -1000 <= value <= 1000
            )  # Large plage pour accepter différentes unités
    ...
    def _estimate_greek_value(
        self, option_data: Dict[str, Any], greek_name: str
    ) -> float:
            ...
            if isinstance(greeks, dict):
                ...
                if bid_iv > 0 or ask_iv > 0:
                    iv = (
                        ...
                        else (bid_iv or ask_iv)
                    )
                    logger.debug(
                        f"   ✅ IV {symbol}: {iv:.4f} (bid/ask: {bid_iv:.4f}/{ask_iv:.4f})"
                    )
                    return iv
    ...
    def _create_contract_from_data(
        self, option_data: Dict[str, Any], underlying: str
        ...
        try:
            ...
            contract = OptionsContract(
                ...
                change_percentage=self._safe_float(
                    option_data.get("change_percentage")
                ),
...
if __name__ == "__main__":
    ...
    if underlying:
        ...
        print(
            f"  Changement: {underlying['change']:+.2f} ({underlying['change_percentage']:+.2f}%)"
        )
    ...
    if expirations:
        ...
        if calls:
            ...
            for call in calls:
                print(
                    f"  {call.strike:.0f} - Vol: {call.volume:,} - OI: {call.open_interest:,} - Last: ${call.last:.2f}"
                )
        ...
        if puts:
            ...
            for put in puts:
                print(
                    f"  {put.strike:.0f} - Vol: {put.volume:,} - OI: {put.open_interest:,} - Last: ${put.last:.2f}"
                )
