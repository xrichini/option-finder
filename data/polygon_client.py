...
class PolygonClient:
    ...
    def __init__(self, api_key: str, base_url: str = "https://api.polygon.io"):
        ...
        self.session.headers.update(
            {"Authorization": f"Bearer {api_key}", "User-Agent": "squeeze-finder/1.0"}
        )
    ...
    def _make_request(self, endpoint: str, params: Dict = None) -> Dict:
        ...
        try:
            ...
            if data.get("status") != "OK":
                print(
                    f"⚠️ API Warning: {data.get('status')} - {data.get('error', 'Unknown error')}"
                )
            ...
            if response.status_code == 429:
                print(
                    "❌ Rate limit exceeded. Consider upgrading your Polygon.io plan."
                )
    ...
    def get_option_quotes(
        self, option_ticker: str, timestamp: Optional[str] = None
    ...
    def analyze_unusual_volume(
        self, tickers: List[str], days_back: int = 30, volume_threshold: float = 2.0
    ) -> Dict[str, Dict]:
        """
        ...
        for ticker in tickers:
            try:
                ...
                if latest["volume_5d_std"] > 0:
                    volume_z_score = (
                        latest["volume"] - latest["volume_5d_avg"]
                    ) / latest["volume_5d_std"]

                if latest["transactions_5d_std"] > 0:
                    transactions_z_score = (
                        latest["transactions"] - latest["transactions_5d_avg"]
                    ) / latest["transactions_5d_std"]

                # Check for unusual activity
                is_unusual = (
                    volume_z_score >= volume_threshold
                    or transactions_z_score >= volume_threshold
                )
                ...
                if is_unusual:
                    ...
                    print(
                        f"🔥 {ticker}: Volume Z-score {volume_z_score:.2f}, Transactions Z-score {transactions_z_score:.2f}"
                    )
        ...
        print(
            f"📊 Analysis complete: {len(analysis_results)} tickers with unusual activity"
        )
        return analysis_results

    def get_high_volume_options(
        self, underlying_tickers: List[str], min_volume_threshold: int = 1000
    ) -> Dict[str, List[Dict]]:
        """
        Find high volume options for given underlying tickers
        """
        print(
            f"🎯 Finding high volume options for {len(underlying_tickers)} underlyings..."
        )
        ...
        for ticker in underlying_tickers:
            try:
                ...
                contracts = self.get_option_contracts(
                    underlying_ticker=ticker, limit=500  # Limit to avoid rate limits
                )
                ...
                near_term_contracts = [
                    c for c in contracts if c.expiration_date <= cutoff_date
                ]
                ...
                if ticker_options:
                    ...
                    print(
                        f"📈 {ticker}: Found {len(ticker_options)} high volume options"
                    )
        ...
        print(
            f"✅ High volume options analysis complete: {len(high_volume_options)} underlyings with activity"
        )
...
def test_polygon_client():
    ...
    try:
        ...
        bars = client.get_stock_aggregates(
            "SPY", from_date=datetime.now() - timedelta(days=5)
        )
