...
class HybridDataService:
    ...
    def __init__(self, enable_polygon: bool = True):
        ...
        logger.info(f"   Polygon.io (historique): {'✅' if self.polygon_enabled else '❌'}")
    ...
    async def get_historical_volume_data(self, symbol: str, days: int = 30) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.debug(f"📊 Données historiques Polygon.io pour {symbol}: {len(stock_bars)} jours")
    ...
    async def get_multiple_underlying_prices(self, symbols: List[str]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            logger.info(f"📊 Récupération prix multiples: {', '.join(symbols_to_fetch)}")
            quotes_data = self.tradier_client.get_multiple_underlying_quotes(symbols_to_fetch)
            ...
            if quotes_data:
                logger.info(f"✅ Récupéré {len(quotes_data)} prix sous-jacents en 1 appel API")
    ...
    async def enrich_opportunity_with_hybrid_data(self, opp: OptionsOpportunity, preloaded_underlying_price: Optional[float] = None) -> HybridMetrics:
        """
        ...
        try:
            ...
            if preloaded_underlying_price is not None:
                ...
                logger.debug(f"💾 Utilisation prix pré-chargé pour {opp.underlying_symbol}: ${underlying_price}")
            else:
                underlying_price = await self.get_underlying_price(opp.underlying_symbol)
                logger.debug(f"📊 Récupération prix individuel pour {opp.underlying_symbol}: ${underlying_price}")
            ...
            greeks_available = all([
                opp.bid > 0, opp.ask > 0, opp.last > 0
            ])
            
            # Vérifier si on a des Greeks valides
            greeks_count = sum(1 for g in [delta, gamma, theta, vega] if g is not None and g != 0)
            ...
            if self.polygon_enabled:
                historical_data = await self.get_historical_volume_data(opp.underlying_symbol, 30)
            ...
            logger.debug(f"🔄 Analyse hybride {opp.option_symbol}: score={hybrid_score:.1f}")
            return metrics
    ...
    def _calculate_historical_score(self, historical_data: Dict[str, Any], opp: OptionsOpportunity) -> float:
