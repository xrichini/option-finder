    ...
    async def screen_options_hybrid(
        ...
        try:
            ...
            if progress_callback:
                await progress_callback(0, len(symbols), "INIT", 
                                      "Initialisation du screening hybride...")
            ...
            async def base_progress_callback(current: int, total: int, message: str):
                if progress_callback:
                    symbol = message.split()[-1].replace('...', '') if 'Analyse' in message else 'N/A'
                    await progress_callback(current, total, symbol, f"Screening base: {message}")
            ...
            if progress_callback:
                await progress_callback(len(base_opportunities), len(base_opportunities),
                                      "PREP", "Préparation enrichissement hybride optimisé...")
            
            # Récupération optimisée de TOUS les prix sous-jacents en 1 seul appel API
            underlying_symbols = list(set(opp.underlying_symbol for opp in base_opportunities))
            logger.info(f"🚀 Optimisation: Récupération de {len(underlying_symbols)} prix sous-jacents en 1 appel")
            
            underlying_prices = await self.hybrid_service.get_multiple_underlying_prices(underlying_symbols)
            ...
            for i, opp in enumerate(base_opportunities):
                ...
                try:
                    ...
                    enriched_opp = {
                        ...
                        "option_type_emoji": "📈" if option_type_display == "CALL" else "📉",  # Emoji visuel
                        "strike": opp.strike,
                        "expiration": getattr(opp, 'expiration', None) or getattr(opp, 'expiration_date', None),
                        "dte": opp.dte,
            ...
            logger.info(f"   Polygon.io actif: {'✅' if self.hybrid_service.polygon_enabled else '❌'}")
    ...
    async def get_hybrid_recommendations(
        ...
        try:
            ...
            if not symbols:
                ...
                )[:50]  # Limiter à 50 symboles pour l'analyse
            
            logger.info(f"🤖 Génération de recommandations hybrides sur {len(symbols)} symboles")
