...
from data.short_interest_scraper import (
...
class ShortInterestRequest(BaseModel):
    ...
    exchange: str = Field(
        default="all", description="Exchange à cibler (all, nasdaq, nyse, amex)"
    )
    enable_prefiltering: bool = Field(
        default=True, description="Activer le pré-filtrage par critères de marché"
    )
    min_market_cap: int = Field(
        default=100_000_000, description="Capitalisation minimum en USD"
    )
    min_avg_volume: int = Field(default=500_000, description="Volume moyen minimum")
    min_short_interest: float = Field(
        default=20.0, description="Short interest minimum en pourcentage"
    )
    max_price: Optional[float] = Field(
        default=None, description="Prix maximum par action"
    )
    excluded_sectors: Optional[List[str]] = Field(
        default=None, description="Secteurs à exclure"
    )
...
async def get_short_interest_stocks(
    exchange: str = Query(
        default="all", description="Exchange (all, nasdaq, nyse, amex)"
    ),
    enable_prefiltering: bool = Query(
        default=True, description="Activer le pré-filtrage"
    ),
    ...
    min_short_interest: float = Query(
        default=20.0, description="Short interest minimum %"
    ),
    ...
    try:
        ...
        if enable_prefiltering:
            ...
            filtered_stocks = scraper.filter_stocks_by_criteria(
                enriched_stocks, filter_params
            )
        ...
        logger.info(
            f"✅ Short interest scraping terminé: {len(response_stocks)} stocks en {execution_time:.2f}s"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Erreur lors du scraping: {str(e)}"
        )
...
async def scan_with_short_interest_symbols(
    ...
    try:
        ...
        logger.info(
            f"✅ Pipeline terminé: {len(screening_result.get('opportunities', []))} opportunités en {execution_time:.2f}s"
        )
...
async def scan_with_ai_classification(
    ...
    min_squeeze_probability: float = Query(
        default=60.0, description="Probabilité squeeze minimum"
    ),
):
    """
    Pipeline complet: Short Interest → Filtrage → Screening Options
    ...
    try:
        logger.info(
            "🤖 Début du pipeline avancé: Short Interest → Options → Classification IA"
        )
        ...
        si_data_list = await scraper.get_high_short_interest_data(
            exchange=request.exchange, limit=50  # Limite pour l'IA
        )
        ...
        opportunities = (
            ...
            else screening_result.get("opportunities", [])
        )
        ...
        logger.info(f"📊 {len(opportunities)} opportunités options trouvées")

        # Étape 3: IA classification removed (module deleted during cleanup)
        # Returning opportunities directly without AI analysis
        final_opportunities = opportunities
        ...
        if ai_classified_opportunities:
            ...
            squeeze_probs = [
                opp.get("ai_squeeze_probability", 0)
                for opp in ai_classified_opportunities
            ]
        ...
        logger.info(
            f"✨ Pipeline IA terminé: {len(final_opportunities)} opportunités finales en {execution_time:.2f}s"
        )
...
# Route de test/santé
# Note: AI test endpoint disabled - AI classifier module removed during cleanup
# @short_interest_router.get("/test-ai")
# async def test_ai_classification():
#     """Test endpoint pour vérifier le fonctionnement de l'IA"""
#     # Disabled - uses deleted AIShortInterestClassifier module
