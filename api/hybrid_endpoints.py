...
async def screen_options_hybrid_background(
    ...
    request: HybridScreeningRequest
):
    """
    ...
    try:
        ...
        def calculate_type_stats(type_opportunities):
            ...
            return {
                ...
                "avg_hybrid_score": sum(o.get("hybrid_score", 0) for o in type_opportunities) / len(type_opportunities),
                "avg_volume": sum(o.get("volume", 0) for o in type_opportunities) / len(type_opportunities),
                "avg_oi": sum(o.get("open_interest", 0) for o in type_opportunities) / len(type_opportunities),
                "best_opportunity": max(type_opportunities, key=lambda x: x.get("hybrid_score", 0))
            }
...
async def get_symbol_historical_analysis(symbol: str, days: int = Query(default=30, ge=1, le=90)):
    """
    ...
    try:
        ...
        historical_data = await hybrid_service.hybrid_service.get_historical_volume_data(
            symbol=symbol.upper(),
            days=days
        )
...
async def get_underlying_price(symbol: str):
    ...
    try:
        ...
        if not quote_data:
            raise HTTPException(
                status_code=404,
                detail=f"Prix non disponible pour {symbol}"
            )
