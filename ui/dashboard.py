...
class OptionsDashboard:
    ...
    def run(self):
        ...
        <div style="text-align: center; padding: 1rem; background: linear-gradient(90deg, #1f4e79, #2e7d32); border-radius: 10px; margin-bottom: 2rem;">
            <h1 style="color: white; margin: 0;">🐋 Options Whale Screener</h1>
    ...
    def render_sidebar(self):
        ...
        st.session_state.max_dte = st.sidebar.slider(
            "DTE Maximum", 
            min_value=1, max_value=45, value=Config.DEFAULT_DTE, step=1,
            help="Days To Expiration - Plage de recherche des expirations"
        )

        st.session_state.min_volume = st.sidebar.number_input(
            "Volume minimum (1J)", 
            min_value=100, max_value=10000, value=Config.MIN_VOLUME_THRESHOLD, step=100
        )

        st.session_state.min_oi = st.sidebar.number_input(
            "Open Interest minimum", 
            min_value=100, max_value=5000, value=Config.MIN_OPEN_INTEREST_THRESHOLD, step=100
        )

        st.session_state.min_whale_score = st.sidebar.slider(
            "Score Whale minimum", 
            min_value=50, max_value=95, value=Config.MIN_WHALE_SCORE, step=5,
        ...
        st.session_state.min_short_interest = st.sidebar.slider(
            ...
            min_value=10, max_value=80, value=int(Config.DEFAULT_SHORT_INTEREST_THRESHOLD), step=5
        )
            ...
            if results:
                ...
                with col1:
                    ...
                    fig1 = px.bar(
                        df_display.head(10), 
                        x='Symbol', 
                        y=[int(v.replace(',', '')) for v in df_display.head(10)['Volume (1D)']],
                ...
                st.info("🔍 Aucune opportunité détectée avec les critères actuels. Essayez d'ajuster les paramètres.")
        else:
            st.info("👆 Configurez vos paramètres et cliquez sur **SCANNER** pour commencer")
    ...
    def render_short_interest_tab(self):
        ...
        if hasattr(st.session_state, 'trigger_scan') or st.button("🔍 Analyser Short Interest"):
        ...
        if hasattr(st.session_state, 'short_results'):
            ...
            if short_data:
                ...
                df_short['Short %'] = df_short['short_percent'].apply(lambda x: f"{x:.1f}%")
                df_short['Short Ratio'] = df_short['short_ratio'].apply(lambda x: f"{x:.1f}")

                # Tableau
                display_cols = ['symbol', 'Short %', 'Short Ratio', 'Market Cap']
                st.dataframe(
                    df_short[display_cols].rename(columns={
                        'symbol': 'Symbol',
                ...
                if len(short_data) > 1:
                    fig = px.bar(
                        ...
                        labels={'short_percent': 'Short Interest %', 'symbol': 'Symbol'},
                        color='short_percent',
                ...
                if hasattr(st.session_state, 'scan_results'):
                    ...
                    if combo_symbols:
                        st.warning(f"🚨 **ALERTE COMBO** - Symboles avec Big Calls ET High Short Interest: {', '.join(combo_symbols)}")
            else:
